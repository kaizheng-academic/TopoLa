...
from utils_TopoLa import eval_scib_metrics
...
def eval_testdata(
    ...
    data_dict = {'labs': celltypes_labels}

    # 保存到 mat 文件
    scipy.io.savemat(config.dataset_name+'_labs.mat', data_dict)
    ...
    if "cls" in include_types:
        ...
        try:
            ...
            traceback.print_exc()
            logger.error(e)
        adjacency_sparse = np.array(pd.read_csv("adjacency_sparse.csv", header=None).values)
...
def eval_testdata_TopoLa(
    model: nn.Module,
    adata_t: AnnData,
    include_types: List[str] = ["cls"],
    TopoLa_lambda_values: List[float] = [1e-5]
) -> Optional[Dict]:
    """Evaluate the model on test dataset of adata_t and select the best TopoLa_lambda."""
    model.eval()

    # Copy adata_t to avoid reuse of previously computed results stored in adata_t
    adata_t = adata_t.copy()
    ...
    if "cls" in include_types:
        ...
        best_avg_bio = -float('inf')
        best_TopoLa_lambda = None
        avg_bio_values = []

        for TopoLa_lambda in TopoLa_lambda_values:
            print(TopoLa_lambda)
            with torch.no_grad(), torch.cuda.amp.autocast(enabled=config.amp):
                cell_embeddings = model.encode_batch(
                    all_gene_ids,
            ...
            results = eval_scib_metrics(adata_t,TopoLa_bool=True,TopoLa_alpha=TopoLa_lambda)
            
           

            avg_bio = results.get('avg_bio', 0.0)
            avg_bio_values.append(avg_bio)

            if avg_bio > best_avg_bio:
                best_avg_bio = avg_bio
                best_TopoLa_lambda = TopoLa_lambda
                best_results = results.copy()
                cell_embeddings = pd.read_csv("TopoLa.csv", header=None).values
                best_cell_embeddings = cell_embeddings[1:]
  

        logger.info(f"Best TopoLa_lambda: {best_TopoLa_lambda}, Best avg_bio: {best_avg_bio}")

        # Update adata_t with the best cell embeddings
        adata_t.obsm["X_scGPT"] = best_cell_embeddings
        ...
        best_results["celltype_umap"] = fig
...
from utils_TopoLa import eval_scib_metrics
# Load the best model
best_model_path ="/root/autodl-tmp/scGPT/tutorials/save/PBMC_raw/best_model.pt"
model.load_state_dict(torch.load(best_model_path))

# Evaluate on test data
results = eval_testdata_TopoLa(
    model,
    adata_t=adata_sorted if per_seq_batch_sample else adata,
    include_types=["cls"],
)
