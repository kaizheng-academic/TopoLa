...
if opts.load is not None:
    # 加载预训练模型的state_dict
    state_dict = torch.load(opts.load)
    
    # 检查state_dict的键是否有'module.'前缀
    # 如果没有，则添加这个前缀
...
def eval(net, loader, ep):
    ...
    # 获取数据集的名称
    dataset_name = opts.dataset.__name__

    # 保存embeddings到CSV文件
    with open(f'{dataset_name}_embeddings.csv', 'w') as f:
        for embedding in embeddings_all:
            embedding_str = ','.join(map(str, embedding.numpy()))
            f.write(f"{embedding_str}\n")

    # 保存labels到CSV文件
    with open(f'{dataset_name}_labels.csv', 'w') as f:
