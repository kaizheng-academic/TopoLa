...
class RkdDistance_TopoLa(nn.Module):
    def __init__(self):
        super(RkdDistance_TopoLa, self).__init__()
        self.lambda_value = nn.Parameter(torch.tensor(0.001).cuda())
    def get_lambda_value(self):
        return self.lambda_value.item()
    def transform_matrix(self, temp):
        
        if len(temp.shape) == 2:
            temp_T = temp.t()
            product = torch.mm(temp, temp_T)
            identity_matrix = torch.eye(product.size(0), device=temp.device, dtype=temp.dtype)
            scaled_identity = (1 / self.lambda_value) * identity_matrix
            ...
            batch_size, n, m = temp.size()
            results = []
            for i in range(batch_size):
                matrix = temp[i]
                transformed = self.transform_matrix(matrix)
    ...
    def forward(self, student, teacher):
        ...
        with torch.no_grad():
            teacher = self.transform_matrix(teacher)
            mask = torch.triu(teacher, diagonal=1) != 0
            t_d = teacher[mask]
            #t_d = t_d / torch.norm(t_d)


           

        student = self.transform_matrix(student)
        mask = torch.triu(student, diagonal=1) != 0
        d = student[mask] 
        #d = d / torch.norm(d)      
